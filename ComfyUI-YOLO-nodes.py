...
class UltralyticsModelLoader:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "model_name": (folder_paths.get_filename_list("ultralytics"), {"tooltip": "Path of yolo models."}),
            },
            "optional": {
                # "model_name": (os.path.join(models_dir, "ultralytics"), {"tooltip": "path of yolo models."})
                #     (
                #     [
                ...
                #         "yolov13n.pt", "yolov13s.pt", "yolov13m.pt", "yolov13l.pt", "yolov13x.pt",
                #         "mobile_sam.pt"
                #     ],
                # ),
            },
...
class UltralyticsCheckCategory:
    """
    ComfyUI 节点：检查一个浮点数列表（转换为整数后）是否包含指定的整数值。
    """
    def __init__(self):
        pass
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "labels": ("LABELS", {"default": [0.0, 1.0, 24.0]}),
                # 输入 2: 整型数值
                # 范围 0-79 可以在此处通过 "min" 和 "max" 指定，但 ComfyUI 的 LIST 类型通常用作通用容器
                # 对于标准的数值输入，您可以使用 INT
                "category_index": ("INT", {"default": 0, "min": 0, "max": 79, "step": 1}),
            },
        }
    ...
    # 告诉 ComfyUI 这是一个普通节点 (FUNCTION)
    FUNCTION = "check_list" 
    
    # 节点的显示名称
    CATEGORY = "Ultralytics/Utils" 

    def check_list(self, labels, category_index):
        """
        执行节点的主要逻辑：
        1. 将输入列表中的所有浮点数元素转换为整数。
        2. 检查 category_index 是否存在于转换后的整数列表中。
        """
        try:
            int_list = [int(x) for x in labels]
        except (TypeError, ValueError) as e:
            print(f"ListContainsInteger 节点错误: 列表元素转换失败。请确保列表只包含数字。错误信息: {e}")
            return (False,)
        is_contained = category_index in int_list
        
        # 节点函数必须返回一个包含所有输出值的元组
        return (is_contained,)

class UltralyticsListIndexToNames:
    """
    ComfyUI 节点：将浮点数列表（作为索引）转换为对应的 COCO 类别名称列表。
    """
    def __init__(self):
        self.class_names = coco_classes
        self.max_index = len(self.class_names) - 1

    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
                "labels": ("LABELS", {"default": [0.0, 0.0, 67.0]}),
            },
        }
    ...
    FUNCTION = "get_names_from_indices"
    CATEGORY = "Ultralytics/Utils" 
    
    def get_names_from_indices(self, labels):
        """
        执行节点的主要逻辑：
        1. 将输入的浮点数列表转换为整数索引列表。
        2. 遍历整数索引，从 coco_classes 列表中获取对应的字符串名称。
        """
        output_names = []

        for index_float in labels:
            try:
                index_int = int(index_float)
                
                if 0 <= index_int <= self.max_index:
                    # 获取对应的类别名称
                    name = self.class_names[index_int]
                    output_names.append(name)
                else:
                    # 如果索引超出范围，添加一个占位符或警告
                    output_names.append(f"INVALID_INDEX_{index_int}")
                    print(f"ListIndexToNames 警告: 索引 {index_int} 超出 COCO 类别列表范围 (0-{self.max_index})。")
            
            except (TypeError, ValueError) as e:
                output_names.append("CONVERSION_ERROR")
                print(f"ListIndexToNames 错误: 列表元素 {index_float} 转换失败。错误信息: {e}")
...
NODE_CLASS_MAPPINGS = {
    ...
    "ViewText": ViewText,
    "UltralyticsCheckCategory": UltralyticsCheckCategory,
    "UltralyticsListIndexToNames": UltralyticsListIndexToNames
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "ViewText": "View Text",
    "UltralyticsCheckCategory": "Check Category",
    "UltralyticsListIndexToNames": "Map Index to Category Name"
}
